...
public class Part02_Variable02 {
    public static void main(String[] args){
         ...
         *                      저장형식에서 부호비트가 0이면 양수 ex) 0:7bit = 2의 7제곱 = 128개 = 0 ~ 127
         *                      저장형식에서 부호비트가 1이면 음수 ex) 1:7bit = 2의 7제곱 = 128개 = -1 ~ -128
         *                      * char(문자) 타입만 제외하고 모두 * 부호비트 *를 붙인 저장형식을 사용합니다.(음수와 양수를 나누기 위해)
         ...
         *
         *          - 표현범위(3/3)
         *              float 와 double 타입의 저장 형식과 범위에 대해서 알아보겠습니다.
         *
         *              1. float, 4byte = 32bit
         *                  1) 저장 가능한 값의 범위 : 양수 = 1.4E-45 ~ 3.4E38 (E = 10의 몇제곱인지를 나타내기 위한 기호)
         *                                          음수 = -1.4E-45 ~ -3.4E38
         *                      <--[-3.4 * 10의 38제곱]-->[-1.4 * 10의 -45 ~ 1.4 * 10의 -45]<--[3.4 * 10의 38제곱]-->
         *                                       가운데의 식의 범위는 float타입으로는 표현할 수 없는 범위입니다.
         *                     그런데 float 타입은 integer타입과 같은 4byte인데, 어떻게 해서 이렇게 큰 수를 표현할 수 있는 것일까요?
         *                     그 이유는 float 타입의 저장 형식에 있습니다.
         *
         *                  2) 저장 형식 : |S(부호)|E(8)(지수)|M(23)(가수)|
         *                      3.4(가수) * 10의 38제곱(지수)
         *                      이와 같은 저장 형식으로 인해 같은 크기의 정수형 타입보다 훨씬 더 큰 범위의 수를 표현할 수 있는 것입니다.
         *                      대신에 정수형과 달리 실수형은 원래 저장하려던 값과 실제로 저장된 값 사이에 오차가 발생할 수 있습니다.
         *                      이 오차는 정밀도(오차 없는 자리수)까지는 정확하게 표현 가능하고 이 아래로부터는 오차가 발생할 수 있습니다.
         *                      이 정밀도를 결정하는 것은 가수의 자릿수입니다.
         *                          float 타입의 가수는 23자리이지만, 정규화를 통해 24자리 까지 저장할 수 있습니다.
         *                          "10의 7제곱 < 2의 24제곱 < 10의 8제곱" 이기 때문에 float의 정밀도가 7자리인것입니다.
         *              1. double, 8byte = 64bit
         *                  1) 저장 가능한 값의 범위 : 양수 = 4.9E-324 ~ 1.8E308
         *                                          음수 = -4.9E-324 ~ -1.8E308
         *
         *                  2) 저장 형식 : |S|E(11)|M(52)|
         *                  double은 말 그대로 가수의 자릿수가 float의 약 2배이기 때문에 정밀도도 float의 약 2배인 15자리 입니다.
         *
         *              * 실수를 저장할 변수의 타입을 선택할 때, 단순히 저장가능한 값의 범위만으로 타입을 선택해서는 안됩니다.
         *                정확한 계산을 수행해야 할때는 정밀도가 중요합니다. 정밀도가 중요한 경우에는 float보다는 double을 사용해야만 합니다.
         *                10진수로 7자리 정도의 정밀도는 그리 높은게 아니라 실수형에서는 double이 default타입입니다.
         *
         *
